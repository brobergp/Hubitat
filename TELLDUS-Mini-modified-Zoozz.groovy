 ...
 *  Changelog:
 *
 *    V1.0 Telldus-Modifications started (17/5 -2019) Broberg
 *    - Missing fingerprint
 *    - Currency symbol wrong
 *    - Accelerator Sensor not available
 *    - Helt Check not available
 *    
 *
 *
 *
 *    Original Driver code for SmartThing : https://github.com/krlaframboise/SmartThings/blob/master/devicetypes/krlaframboise/zooz-power-switch.src/zooz-power-switch.groovy
 *
...
private getMeterPower() { 
	return getMeterMap("power", 2, "W", 2300, settings?.displayPower != false)
}

private getMeterVoltage() { 
	return getMeterMap("voltage", 4, "V", 255, settings?.displayVoltage != false) 
}
...
def ping() {
	logDebug "ping()..."
	sendHubCommand([
		new hubitat.device.HubAction(switchBinaryGetCmd())
	], 100)
	return []
}
...
private getOverloadProtectionParam() {
	return createConfigParamMap(1, "Overload Protection", 1, ["Disabled (NOT RECOMMENDED)":0, "Enabled${defaultOptionSuffix}":1], "overloadProtection")
}

private getPowerFailureRecoveryParam() {
	return createConfigParamMap(2, "Power Failure Recovery", 1, ["Remember last status${defaultOptionSuffix}":0, "Turn On":1, "Turn Off":2], "powerFailureRecovery")
}

private getOnOffNotificationsParam() {
	return createConfigParamMap(3, "On/Off Notifications", 1, null, null, 2) // 2:Manual switch only
}

private getLedIndicatorParam() {
	return createConfigParamMap(4, "LED Power Consumption Indicator", 1, ["Always Show${defaultOptionSuffix}":0, "Show when On (FIRMWARE 2.0)":1, "Show for 5 seconds when turned on or off":2, "Always Off (FIRMWARE 2.0)":3], "ledIndicator")
}
...
private getPowerValueChangeParam() {
	return createConfigParamMap(16, "Power Report Value Change", 2, getPowerValueOptions(), "powerValueChange")
}

private getPowerPercentageChangeParam() {
	return createConfigParamMap(17, "Power Report Percentage Change", 1, getPercentageOptions(10, "No Reports"), "powerPercentageChange")
}

private getPowerReportIntervalParam() {
	return createConfigParamMap(101, "Power Reporting Interval", 4, getIntervalOptions(30, "No Reports"), "powerReportingInterval")
}

private getEnergyReportIntervalParam() {
	return createConfigParamMap(102, "Energy Reporting Interval", 4, getIntervalOptions(300, "No Reports"), "energyReportingInterval")	
}

private getVoltageReportIntervalParam() {
	return createConfigParamMap(103, "Voltage Reporting Interval", 4, getIntervalOptions(0, "No Reports"), "voltageReportingInterval")	
}

private getElectricityReportIntervalParam() {
	return createConfigParamMap(104, "Electrical Current Reporting Interval", 4, getIntervalOptions(0, "No Reports"), "electricityReportingInterval")	
}
